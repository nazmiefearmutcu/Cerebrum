...
def run_sweep(exposures_list=(1, 2, 4), seeds=(0, 1, 2, 3, 4), n_items=7, vocab=10):
    ...
    for E in exposures_list:
        ...
        out["grail"][E] = float(np.mean(g)); out["grail_raw"][E] = g
...
def run_length_sweep(n_items_list=(7, 15, 25), seeds=(0, 1, 2, 3, 4), exposures=1):
    ...
    for N in n_items_list:
        ...
        out["grail"][N] = float(np.mean(g)); out["grail_raw"][N] = g
...
if __name__ == "__main__":
    ...
    print(f"{'exposures':>9}  {'GRAIL-grid':>18}  {'flat-prior':>18}  {'backprop-MLP':>18}")
    for E in sorted(res["grail"]):
        print(f"{E:>9}  {fmt_ci(res['grail_raw'][E]):>18}  "
    ...
    print(f"{'N_items':>9}  {'GRAIL-grid':>18}  {'flat-prior':>18}  {'backprop-MLP':>18}")
    for N in sorted(resN["grail"]):
        print(f"{N:>9}  {fmt_ci(resN['grail_raw'][N]):>18}  "
